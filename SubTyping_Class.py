...
class PeaksSubtypes(strax.Plugin):
    ...
    def mark_with_s1(self,
        ...
        for i in tqdm(range(len(s1s_t)),file=sys.stdout):
            ...
            if len(s2s_a[ps2_cand_cond]):
                ...
                if max_val>=ls2_threshold:
                    ...
                    # pS2 located
                    whole_window_cond = (s2s_t>s1s_t[i])&(s2s_t<=s2s_t[ps2_cand_cond][max_ind]+2*full_dt)
                    s2_mask_tmp = s2_mask[whole_window_cond]
                    s2_mask_tmp[(s2s_a[whole_window_cond]>=np.max([ls2_threshold,other_ls2*max_val]))
    ...
    def mark_s2s(self,
        ...
        while len(dmask[dmask==6])>0:
            ...
            if to_print%100 == 0:
                ...
                print(f'\r{len(dmask[dmask==6])}/{ori_l}', end = "")
                sys.stdout.flush()
            ...
            if start_s2_a >= ls2_threshold:
                ...
                if len(pa[ps2_cand_cond]):
                    ...
                    if max_val>=ls2_threshold:
                        ...
                        mask_tmp = dmask[whole_window_cond]
                        mask_tmp[(pa[whole_window_cond]>=np.max([ls2_threshold,other_ls2*max_val]))
